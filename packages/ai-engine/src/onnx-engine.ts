...
export class OnnxEngine extends Engine {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      console.log(`[AI] Model loaded: ${backendInfo}${threadInfo} in ${timeStr}`);
  ...
  protected async analyzePosition(
    ...
    const binTensor = new ort.Tensor('float32', bin_input, [1, 22, size, size]);
  ...
  async analyzeBatch(
    ...
    */
    ...
    // Run inference
    const binTensor = new ort.Tensor('float32', bin_input, [batchSize, 22, size, size]);
    const globalTensor = new ort.Tensor('float32', global_input, [batchSize, 19]);
